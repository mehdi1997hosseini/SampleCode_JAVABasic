...
import basicvalues.BasicValues;
...
public class BaseDaoImpl<E extends BaseEntity> implements BaseDao<E> {
    protected Connection connection = DataSource.getConnection();

    protected Class<E> entityClass;

    protected BaseDaoImpl() {
        try {
            Type[] actualTypeArguments = ((ParameterizedTypeImpl) getClass().getGenericSuperclass()).getActualTypeArguments();
    ...
    public Boolean save(E e) {
        try {
            ...
            queryInsert.add(BasicValues.INSERT).add("TBL_" + entityClass.getSimpleName().toUpperCase()).add("(")
                    .add(e.convertListFieldsToNameColumns(listField)).add(")").add(BasicValues.VALUES).add("(")
                    .add(e.getNumOfBindVariableFromListField(listField)).add(")");
    ...
    public E getById(Long id) {
        ...
        queryGetByID.add(BasicValues.SELECT_ALL).add("TBL_" + entityClass.getSimpleName().toUpperCase()).add("WHERE").add("ID=?");
        try {
    ...
    public Boolean deleteById(Long id) {
        StringJoiner queryGetByID = new StringJoiner(" ");
        queryGetByID.add(BasicValues.DELETE).add("TBL_" + entityClass.getSimpleName().toUpperCase()).add("WHERE").add("ID=?");
        try {
            PreparedStatement preparedStatement = connection.prepareStatement(String.valueOf(queryGetByID));
            preparedStatement.setLong(1, id);
    ...
    public List<E> getAll() {
        String querySelectAll = BasicValues.SELECT_ALL + "TBL_" + entityClass.getSimpleName().toUpperCase();
        try {
    ...
    public List<E> getByProperty(String propertyName, Object values) {
        try {
            E instance = this.entityClass.newInstance();
            List<Field> allFieldsIntoClass = instance.getListDeclaredFieldsInThisClassWithId();
            ...
            String queryGetByProperty = BasicValues.SELECT_ALL + "TBL_" + entityClass.getSimpleName().toUpperCase() + " WHERE " + instance.convertFieldToNameColumn(property) + "=?";
